...
export default function BookingDetailsScreen({ route }: Props) {
  ...
  // =========================================================
  // FETCH FRESH BOOKING FROM BACKEND
  // =========================================================
  ...
  useEffect(() => {
    ...
    const fetchLatest = async () => {
      try {
        console.log(
          "📡 [BookingDetails] Fetching booking:",
          initialBooking.id
        );

        const response = await getCustomerBooking(
          String(initialBooking.id)
        );

        console.log(
          "✅ [BookingDetails] Backend booking:",
          response
        );

        if (response) {
          setBooking(response.item as typeof initialBooking);
        }
      } catch (error) {
        console.error(
          "❌ [BookingDetails] Failed to fetch booking:",
          error
        );
      }
    };
  ...
  // Cancellation fee is now calculated by the backend.
  // Keep embedded booking/service fee if it is already present.
  useEffect(() => {
    if (!services || services.length === 0) return;

    const embeddedFee = services.find(
      (s: any) =>
        s.cancellation_fee !== undefined &&
        s.cancellation_fee !== null
    );

    if (embeddedFee) {
      setServiceCancellationFee(
        Number(embeddedFee.cancellation_fee)
      );
    }
  }, [services]);
  ...
  const handleCancelBooking = async () => {
    ...
    try {
      console.log(
        "📡 [BookingDetails] Getting cancellation details:",
        booking.id
      );

      const response =
        await getCustomerCancellationDetails(
          String(booking.id)
        );

      console.log(
        "✅ [BookingDetails] Cancellation details:",
        response
      );
      ...
      setCancellationDetails(
        response as typeof cancellationDetails
      );

      setShowCancelModal(true);
    } catch (err) {
      console.error(
        "❌ Error fetching cancellation details:",
        err
      );
  ...
  const confirmCancellation = async () => {
    ...
    try {
      const response = await cancelCustomerBooking(
        String(booking.id),
        cancelReason
      );

      console.log(
        "✅ [BookingDetails] Cancellation response:",
        response
      );
      ...
      if (response.item) {
        setBooking(response.item as typeof booking);
      }
  ...
  setReview({
    ...
    comment: response.item.comment ?? "",
      ...
      console.log(
        "No review found or error fetching:",
        err
      );
    }
  };
  ...
  const handleSubmitReview = async (
    ...
    try {
      console.log(
        "📡 [BookingDetails] Submitting review:",
        booking.id
      );

      const response =
        await submitCustomerBookingReview(
          String(booking.id),
          rating,
          comment
        );
      ...
      console.log(
        "✅ [BookingDetails] Review response:",
        response
      );
  ...
  const fetchStaffDetails = async () => {
    ...
    try {
      const response =
        await getCustomerBookingStaff(
          String(booking.id)
        );

      if (response?.item) {
        if (response.item.name) {
          setStaffName(response.item.name);
        }

        if (response.item.phone) {
          setStaffPhone(response.item.phone);
        }
      }
    } catch (error) {
      console.error(
  ...
  const onRefresh = async () => {
    ...
    try {
      console.log(
        "📡 [BookingDetails] Refreshing booking:",
        booking.id
      );
      ...
      if (response?.item) {
        setBooking(
          response.item as typeof booking
        );

        const status =
          response.item.work_status;

        setIsEligibleToCancel(
