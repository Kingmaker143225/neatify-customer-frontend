...
export default function CheckoutScreen({ route }: Props) {
  ...
  console.error(
    ...
    try {
      /* =========================================================
         1️⃣ CREATE BOOKING THROUGH FASTAPI BACKEND
         ========================================================= */

      console.log("📡 [Checkout] Creating booking through backend...");
      ...
      const bookingResponse = await createCustomerBooking({
        ...
        phone_number: profile.phone,
        full_address: fullAddress,
        services: checkoutServices.map((service: any) => ({
          id: service.id,
      ...
      console.log("✅ [Checkout] Booking backend response:", bookingResponse);

      if (!bookingResponse || !bookingResponse.success || !bookingResponse.booking_id) {
        throw new Error(bookingResponse?.message || "Failed to create booking");
      }

      const bookingId = bookingResponse.booking_id;
      console.log("✅ [Checkout] Booking created:", bookingId);
      console.log("💰 [Checkout] Backend total:", bookingResponse.total_amount);

      /* =========================================================
         2️⃣ CREATE RAZORPAY ORDER THROUGH FASTAPI
         ========================================================= */

      console.log("📡 [Checkout] Creating Razorpay order through backend...");

      const orderResponse = await createCustomerPaymentOrder(bookingId);
      ...
      // Check if amount is in paise or rupees
      // If backend returns amount in paise (e.g., 49900 for ₹499), use as-is
      // If backend returns amount in rupees (e.g., 499), multiply by 100
      // The backend response should indicate the format
      let amountInPaise = orderResponse.amount;
      // If amount is less than 100, it's likely in rupees, convert to paise
      if (amountInPaise < 100) {
        amountInPaise = Math.round(Number(amountInPaise) * 100);
      }
      ...
      console.log("✅ [Checkout] Razorpay payment response:", payment);

      /* =========================================================
         4️⃣ VERIFY PAYMENT THROUGH FASTAPI
         ========================================================= */

      console.log("📡 [Checkout] Verifying payment through backend...");

      const verificationResponse = await verifyCustomerPayment({
        booking_id: bookingId,
      ...
      console.log("✅ [Checkout] Payment verification response:", verificationResponse);

      if (!verificationResponse || !verificationResponse.success || !verificationResponse.payment_verified) {
      ...
      /* =========================================================
         6️⃣ COUPON / WALLET / LOCAL CLEANUP
         ========================================================= */
...
if (useWallet && walletBalance > 0) {
  ...
  console.error(
    ...
    cartError
      ...
      console.error("❌ [Checkout] Booking/payment failed:", error);
      ...
      setAlertConfig({
        ...
        message: error?.message || "We couldn't process your payment at this time. Please try again later.",
        type: "error",
      });
      setShowAlertModal(true);
    }
