...
//     if (
//       rewards.show_welcome_reward &&
//       rewards.welcome_coupon_code
//     ) {
//       console.log(
//         "🎁 Welcome Reward detected!",
//         rewards.welcome_coupon_code
//       );
...
//       // Tell backend that the popup has been shown
//       await updateCustomerRewards({
//         show_welcome_reward: false,
//       });
//     }
//   } catch (err) {
//     console.error(
//       "Error checking welcome reward:",
//       err
//     );
//   }
// }, []);
...
//   try {
//     console.log(
//       "🔍 Checking Signup Offer metadata..."
//     );

//     const rewards = await getCustomerRewards();

//     const serviceTitle =
//       rewards.signup_service_title;

//     const serviceId =
//       rewards.signup_service_id;

//     const shouldShow =
//       rewards.show_signup_offer_popup;
...
//       // Tell backend that the popup has been shown
//       await updateCustomerRewards({
//         show_signup_offer_popup: false,
//       });
//     }
//   } catch (err) {
//     console.error(
//       "Error checking signup offer:",
//       err
//     );
//   }
// }, []);
...
//   // Search in already loaded backend services list first
//   let matchingSvc = services.find(
//     (s) =>
//       (signupOfferInfo.id &&
//         s.id === signupOfferInfo.id) ||
//       (signupOfferInfo.title &&
//         s.title?.toLowerCase().trim() ===
//           signupOfferInfo.title?.toLowerCase().trim())
//   );

//   // If not found in memory, search through FastAPI backend
//   if (!matchingSvc) {
//     try {
//       console.log(
//         "📡 [Backend] Searching service for signup offer..."
//       );

//       const backendServices =
//         await getCustomerServices();

//       matchingSvc = backendServices.find(
//         (s: any) =>
//           (signupOfferInfo.id &&
//             s.id === signupOfferInfo.id) ||
//           (signupOfferInfo.title &&
...
//       if (matchingSvc) {
//         console.log(
//           "✅ [Backend] Signup offer service found:",
//           matchingSvc.id
//         );
//       } else {
//         console.log(
//           "⚠️ [Backend] Signup offer service not found"
//         );
//       }
//     } catch (e) {
//       console.error(
//         "❌ [Backend] Error querying service for signup offer:",
//         e
//       );
//     }
//   }
...
//   } else {
//     showToast(
//       "40% OFF discount saved! Browse services to select your booking.",
//       "info"
//     );
//   }
// }, [
...
export default function HomeScreen({ navigation }: any) {
  ...
  const scrollRef = useRef<ScrollView>(null); // Main ScrollView ref
  const isProgrammaticScroll = useRef(false);
  const hasShownPopupThisMount = useRef(false);
  const [showGoUp, setShowGoUp] = useState(false);
  const popupSliderRef = useRef<FlatList>(null);
  ...
  const interval = setInterval(() => {
    setPopupIndex((currentIndex) => {
      const nextIndex =
        currentIndex >= appPopups.length - 1
          ? 0
          : currentIndex + 1;

      popupSliderRef.current?.scrollToIndex({
        index: nextIndex,
        animated: true,
      });
  ...
  return () => clearInterval(interval);
}, [popupType, showPopup, appPopups.length]);
...
const fetchPopups = useCallback(async () => {
  if (hasShownPopupThisMount.current) {
    console.log("ℹ️ [Popup] Already shown on this HomeScreen mount");
    return;
  }

  try {
    console.log("🟣 [Popup] Fetching customer popups and offers...");

    const [popupResult, offersResult] = await Promise.allSettled([
      getCustomerPopups(),
      getCustomerOffers(),
    ]);

    const popupData =
      popupResult.status === "fulfilled"
        ? popupResult.value
        : [];
    ...
    console.log("🟣 [Popup] Offers data:", offersData);

    if (popupData.length > 0) {
      setAppPopups(popupData);
      setActiveOffers(offersData); // ✅ ADD THIS

      setPopupIndex(0);
      setPopupType("APP_POPUP");
      setShowPopup(true);

      hasShownPopupThisMount.current = true;

      console.log("🟢 [Popup] APP POPUP OPENED");
      return;
    }
    ...
    if (offersData.length > 0) {
      ...
      console.log("🟢 [Popup] OFFERS POPUP OPENED");
      return;
    }

    console.log("ℹ️ [Popup] No popup or offers available");
  } catch (error) {
    console.error("❌ [Popup] Failed to fetch popup/offers:", error);
  }
}, []);
  useFocusEffect(
    useCallback(() => {
    ...
    <View style={{ width: POPUP_WIDTH }}>
      {item.image_url ? (
        <Image
          ...
          style={popupStyles.appImage}
        />
      ) : (
        <View
          style={[
            popupStyles.appImage,
