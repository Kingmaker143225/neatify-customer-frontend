...
//       (service: any) =>
//         service?.is_welcome_offer_eligible === true
//     );

//     setEligibleServices(
//       eligible.length > 0 ? eligible : services.slice(0, 30)
//     );
//   } catch (err) {
//     console.log(
...
//   try {
//     const data = await sendCustomerOtp(cleanPhone);
...
//       throw new Error(
//         "Login successful but authentication tokens were not returned."
//       );
//     }
...
export default function LoginScreen(props: any) {
  ...
  const fetchEligibleServices = async () => {
    try {
      ...
      const eligible = (services || []).filter(
        (service: any) => service.is_welcome_offer_eligible === true
      );
      ...
      console.log("Error fetching customer services:", err);
      setEligibleServices([]);
    }
  };
  ...
  const checkProfileAndNavigate = async () => {
    try {
      ...
      console.log("📋 Customer profile status:", profileStatus);
  ...
  const handleVerifyOtp = async () => {
    ...
    try {
       ...
       * Existing customer
       *
       * Backend may return access_token directly
  ...
  const handleLogout = async () => {
    ...
    try {
      ...
      /*
       * customerLogout() already clears the local token
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (isLogin) {
        ...
        /*
         * Token is already stored by customerLogin().
         * No need to call supabase.auth.setSession() or supabase.auth.getUser()
         */
        await checkProfileAndNavigate();
      ...
      // If email confirmation is not required, show success
      showAlert({
        type: "success",
        title: "Account Created",
        message: "Your account has been created successfully. Please login to continue.",
