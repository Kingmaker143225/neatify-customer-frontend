...
export async function savePushToken(token: string) {
  ...
  try {
    await saveCustomerPushToken(
      token,
      Platform.OS
    );

    console.log(
      "✅ Push token successfully saved through backend"
    );
  } catch (err) {
    console.error(
      "❌ Error saving push token through backend:",
      err
    );
  }
}
...
export async function removePushToken(
  ...
  try {
    await removeCustomerPushToken(token);

    console.log(
      "✅ Push token successfully removed through backend"
    );
  } catch (err) {
    console.error(
      "❌ Error removing push token through backend:",
      err
    );
  }
}